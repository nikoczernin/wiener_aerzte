...
```{r, fig.width=10, fig.height=12, include=FALSE}
bevölkerung %>% 
  filter(Jahr > 2019) %>% 
...
```{r Population pro Bezirk, include=FALSE}
...
```{r Bevölkerung pro Frauenärzt:in, include=FALSE}
# year = 2024
ärztinnen %>% 
  filter(Jahr == year) %>%
  left_join(bevölkerung, by=c("Bezirk_Nr", "Jahr")) %>% 
...
```{r Linechart Patienten pro Arzt, include=FALSE}
...
```{r Bevölkerung pro Fachärztin, include=FALSE}
...
```{r Top überlastete Krankenhäuser, include=FALSE}
krankenhäuser %>% 
  transmute(
